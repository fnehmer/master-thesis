...
\cite{zhuo2017network} and \cite{nalavade2014}
...
This chapter presents known threats when using the OAuth authorization framework. Section \ref{sec:oauth_threats_and_vulns} elaborates on every threat and vulnerability separately. The list is mainly based on the current draft for security best practices for OAuth from the OAuth task force at IETF and is enriched with various examples, further explanations, and a categorization of types of threats.
...
\begin{table}[!ht]
    ...
    \end{tabular}
\end{table}
\todo{Put vertical names countermeasures}
\todo{Create caption for table}

\subsection{Threats by mitigation responsibility}
\todo{Add the table for threats by mitigation responsibility}
