...
Word2Vec ist eine beliebte Technik im Bereich der natürlichen Sprachverarbeitung (NLP) zur Generierung von Wort-Einbettungen, also Vektorrepräsentationen von Wörtern in einem kontinuierlichen Vektorraum. Von Mikolov et al. im Jahr 2013 eingeführt, basiert Word2Vec auf der Verteilungshypothese, nach der Wörter, die in ähnlichen Kontexten erscheinen, semantisch verwandt sind.

\subsubsection{Continuous Bag of Words (CBOW)}

Das CBOW-Modell ist eine der Architekturen, die von Word2Vec verwendet werden. Bei CBOW besteht das Ziel darin, das Zielwort vorherzusagen, basierend auf seinem Kontext, der aus umgebenden Wörtern besteht. Das Modell lernt, Wörter als dichte Vektoren darzustellen, indem es an einem großen Korpus trainiert wird. Die Architektur umfasst eine versteckte Schicht, die semantische Beziehungen zwischen Wörtern erfasst.

\subsubsection{Skip-Gram}

Im Gegensatz zu CBOW zielt das Skip-Gram-Modell darauf ab, die Kontextwörter vorherzusagen, basierend auf einem Zielwort. Es handelt sich um eine weitere Architektur, die in Word2Vec verwendet wird. Skip-Gram wird oft bevorzugt, wenn große Datensätze verwendet werden, da es auf umfangreichen und vielfältigen Korpora gut abschneidet. Die resultierenden Wort-Einbettungen kodieren semantische Informationen und erfassen Beziehungen zwischen Wörtern.

\subsubsection{Trainingsprozess}

Word2Vec verwendet ein neuronales Netzwerk, um die Wort-Einbettungen zu lernen. Der Trainingsprozess beinhaltet die Anpassung der Gewichte des neuronalen Netzwerks unter Verwendung eines großen Korpus. Das Ziel besteht darin, die Differenz zwischen den vorhergesagten und tatsächlichen Wortrepräsentationen zu minimieren. Die resultierenden erlernten Einbettungen erfassen semantische Beziehungen und ermöglichen verschiedene Anwendungen in der NLP, wie beispielsweise die Messung von Wortähnlichkeiten oder Analogien.

\subsubsection{Anwendungen}

Word2Vec findet weitreichende Anwendungen in Aufgaben der natürlichen Sprachverarbeitung, einschließlich, aber nicht beschränkt auf, Sentimentanalyse, maschinelles Übersetzen und Information Retrieval. Die Fähigkeit, semantische Beziehungen zwischen Wörtern in einem kontinuierlichen Vektorraum zu erfassen, hat es zu einem grundlegenden Werkzeug im Bereich der NLP gemacht.

In den folgenden Kapiteln werden wir tiefer in die Details von Word2Vec eintauchen, seine Anwendungen erkunden und seine Leistung im Kontext unserer Forschungsziele bewerten.
...
The k-means clustering algorithm was chosen as it is one of the most 
